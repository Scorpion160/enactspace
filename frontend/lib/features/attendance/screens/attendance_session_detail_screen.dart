...
class _AttendanceSessionDetailScreenState
  ...
  Future<void> _generateQrToken({bool silent = false}) async {
    ...
    try {
      final token = await _attendanceService.createQrToken(widget.session.id);
      if (!mounted) return;
      setState(() {
        _qrToken = token;
        _qrError = null;
  ...
  void _scheduleQrRefresh(AttendanceQrTokenModel token) {
    _qrRefreshTimer?.cancel();
    final secondsUntilExpiry = token.expiresAt
        .difference(DateTime.now())
        .inSeconds;
    final waitSeconds = [token.rotationSeconds - 2, secondsUntilExpiry - 2]
        .where((value) => value > 0)
        .fold<int>(8, (min, value) {
          if (min == 8) return value.clamp(5, 120);
          return value < min ? value.clamp(5, 120) : min;
        });

    _qrRefreshTimer = Timer(Duration(seconds: waitSeconds), () {
